...
def treeSpace(material, tree, node, num):
    ...
    if tree[node]['#next']:
        for one in tree[node]['#next']:
            if material[one]['datatype'] == 'function':
                treeSpace(material, tree, one, tree[node]["#s"]-4)
            else:
                treeSpace(material, tree, one, tree[node]["#s"])
...
def nodeTxt(material, tree, node):
    if node > '0':
        if material[node]['datatype'] and material[node]['datatype'] in ('module'):
            ...
            return '%s%s = %s.findall("%s")' % (' ' * tree[node]['#s'], material[node]['name'], material[material[node]['pid']]['name'], material[node]['xpath']) + '\n'
            ...
            if material[node]['method'] == 'init':
                if material[node]['default'] is None:
                    val = 'self.%s' % material[material[node]['pid']]['name'] if material[material[node]['pid']]['datatype'] == 'class' or material[str(material[material[node]['pid']]['sid'])]['name'] == '__init__' else '%s' % material[material[node]['pid']]['name']
                else:
