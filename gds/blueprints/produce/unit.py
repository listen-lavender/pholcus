...
from makescript import newunit
...
def unitdetail(uid=None):
    ...
    if request.method == 'GET':
        if uid is None:
            unit = {'id':'', 'unit_name':'', 'datamodel_name':'', 'extra':''}
        else:
            unit = dbpc.handler.queryOne(""" select gu.id, gu.name as unit_name, gdm.name as datamodel_name, gu.extra from grab_unit gu join grab_datamodel gdm on gu.dmid = gdm.id where gu.id = %s; """, (uid,))
