...
class AuthModel(orm.Model):
    ...
    def queryOne(cls, uid, spec, projection={}, sort=[]):
        user = Creator.queryOne({'_id':uid}, projection={'username':1}) or {'username':None}
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth % 2 == 1: # 1 3 5 7 9 11 13 15
            result = orm.Model.queryOne(cls, spec, projection=projection, sort=sort)
        else:
            result = None
        return result
    ...
    def queryAll(cls, uid, spec, projection={}, sort=[], skip=0, limit=10):
        ...
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth % 2 == 1: # 1 3 5 7 9 11 13 15
            result = orm.Model.queryAll(cls, spec, projection=projection, sort=sort, skip=skip, limit=limit)
        else:
            result = None
        return result
    ...
    def count(cls, uid, spec):
        ...
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth % 2 == 1: # 1 3 5 7 9 11 13 15
            result = orm.Model.count(cls, spec)
        else:
            result = None
        return result
    ...
    def insert(cls, uid, obj, update=True, method='SINGLE', forcexe=False, maxsize=MAXSIZE):
        user = Creator.queryOne({'_id':uid}, projection={'username':1}) or {'username':None}
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth > 7: # 8 9 10 11 12 13 14 15
            result = orm.Model.insert(cls, obj, update=update, method=method, forcexe=forcexe, maxsize=maxsize)
        else:
            result = None
        return result
    ...
    def delete(cls, uid, spec):
        user = Creator.queryOne({'_id':uid}, projection={'username':1}) or {'username':None}
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth in (4,5,6,7,12,13,14,15):
            result = orm.Model.delete(cls, spec)
        else:
            result = None
        return result
    ...
    def update(cls, uid, spec, doc):
        user = Creator.queryOne({'_id':uid}, projection={'username':1}) or {'username':None}
        auth = Permit.queryOne({'cid':uid, 'otype':cls.__name__, 'oid':spec.get('_id')}, projection={'authority':1})['authority']
        if user['username'] == 'root' or auth in (2,3,6,7,10,11,14,15):
            result = orm.Model.update(cls, spec, doc)
        else:
            result = None
        return result
...
class Permit(AuthModel):
    ...
    otype = orm.StrField(ddl='varchar', max_length=50, unique='gp')
    authority = orm.IntField(ddl='int', max_length=3)
    desc = orm.StrField(ddl='char', max_length=4)
    status = orm.IntField(ddl='int', max_length=1)
    extra = orm.StrField(ddl='varchar', max_length=300)
