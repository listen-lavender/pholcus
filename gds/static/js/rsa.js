...
var bLt = function(bk, fF, uj) {
    if (!bk || !bk.length || !gj(fF)) return this;
    if ( !! bk.forEach) {
        bk.forEach(fF, uj);
        return this
    }
    for (var i = 0,
    l = bk.length; i < l; i++) fF.call(uj, bk[i], i, bk);
    return this
};
...
(function() {
    ...
    d = d.AES = p.extend({
        bHj: function() {
            ...
            c = a.words,
            d = a.sigBytes / 4,
            a = 4 * ((this.bHz = d + 6) + 1), e = this.bHy = [], j = 0; j < a; j++) if (j < d) e[j] = c[j];
            else {
                var k = e[j - 1];
...
var bHl = window.asrsea(JSON.stringify({'a':'hao', 'b':'kuan'}), bi(["流泪", "强"]), bi(bHd.md), bi(["爱心", "女孩", "惊恐", "大笑"]));
console.log(JSON.stringify({'a':'hao', 'b':'kuan'}));
console.log('----encText----');
console.log(bHl.encText);
console.log('----encSecKey----');
console.log(bHl.encSecKey);
